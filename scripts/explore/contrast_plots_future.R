...
for(v in variables){
  for(c in simulations){
    output <- read_mean_outputvalue(file.path(sim_dir, c), 
                                    years = c(2020:2050), model = "PHENOFIT", output_var = v)
    output$id <- c
    output$var <- v
...
ggsave(filename=file.path(wd, "scripts", "explore", "graphs", scenario, "contrast_indexmaps_fsylvatica_20202050_gfdl.pdf"), 
