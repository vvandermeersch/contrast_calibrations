...
stations_tempo  <- fread(file.path(tempo_dir, "sites.csv")) %>%
  dplyr::select("site id", latitude, longitude)

records_tempo <- fread(file.path(tempo_dir, "pheno-pmp.txt")) %>%
  ...
  reframe(lat = latitude, lon = longitude, year = ANNEE,
          stade.raw = stade,
          stade = str_split(stade.raw, pattern = "-", simplify = TRUE)[,-1],
