...
parameters <- data.frame()
{
  ...
  ## Betula pendula
  par_dir <- "D:/calibrations/phenofit/betula_pendula/1000pres_1000abs/partial/expfrost_flo_mat"
  cal <- c(paste0("subset_",rep(1:2, each = 5),"/cmaes_fit_subset", rep(1:2, each = 5),"_rep", 1:5, ".species"))
  species_parameters <- lapply(cal, function(x){
    species_values <- data.frame(value = read_species_file(file.path(par_dir,x))) %>% 
      mutate_all(as.numeric)
...
expert_parameters <-
  data.frame(
    ...
    var = factor(rep(c("FH[leaf]^{\n    min\n}", "FH[leaf]^{\n    max\n}"), each = 3), levels = c("FH[leaf]^{\n    min\n}", "FH[leaf]^{\n    max\n}")),
    value = c(-3, -3.2, -5.3, -25, -38, -20)
  )
...
parameter_boxplot <- frost_par_sum %>%
  ...
  geom_point(data = expert_parameters, aes(y = value, x = 0.5),
             shape = 18, col = "#577590", fill = "#577590", alpha = 1,
             stroke = 1,  size = 0.9) +
  geom_hline(data = expert_parameters, aes(yintercept = value),
             col = "#577590", alpha = 0.5,
  ...
  ggh4x::facetted_pos_scales(y = list(
    ...
    var == "FH[leaf]^{\n    max\n}" ~ scale_y_continuous(limits = c(-45, 0), breaks = c(-45, -22.5, 0), expand = c(0,0), minor_breaks= c(-11.25, -33.75), guide = "axis_minor")
  )) + 
  coord_cartesian(clip = "off", xlim = c(0.5, 2.5), expand = FALSE)
